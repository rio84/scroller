    ...
    function Scroller(conf){
        ...
        var CONST_KEY_INDEX='itemIndex',
            ...
            _redirect=function(dis){
                var div=_container[0],ori=div.scrollTop,aim=ori+dis;
                if(_scrollMutiple){
                    aim=Math.round(aim/_unitHeight)*_unitHeight;
                }
                div.scrollTop=aim;
            ...
            scrollNext=function(dis){
                ...
                if(div.scrollHeight>div.clientHeight+div.scrollTop){
                    ...
                    _scrollBeforeSendEvent(aim);//看上去像是回调被中断了，没有执行
                    doScroll(div,start,aim,_scrollNextDone);
                }else{
            ...
            scrollPrev=function(dis){
                var div=_container[0],start=div.scrollTop,dis=dis||_scrollDistance,
                    aim=start-dis;//修复chrome 窗口没有blur后scroll位置的问题
                if(start>0){
                    if(_scrollMutiple){
                        aim=Math.round(aim/_unitHeight)*_unitHeight;
                    }
                    _scrollBeforeSendEvent(aim);
                    doScroll(div,start,aim,_scrollPrevDone);
                }else{
