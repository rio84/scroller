/**
	...
	Scroller({
		...
		clickChange:true,
		scrollByMutipleWidth:[true|false]   //是否以一个单元的倍数滚动
	})
	【方法】
	init(index)
...
*/
KISSY.add('script/scroller',function(S){
function Scroller(conf){
	var config={//
		scrollOnEdge:true,
		scrollByMutipleWidth:false
	};
	for(var k in conf){
		config[k]=conf[k];
	}
	var CONST_KEY_INDEX='itemIndex',
		...
		_unitWidth=$(_items[0]).outerWidth(true),
		_scrollDistance=_unitWidth*(config.countPerScroll||Math.floor(_container.width()/_unitWidth)),
		_scrollMutiple=config.scrollByMutipleWidth,
		_canGoPrev=true,
		_canGoNext=true,
		_canScrollPrev=true,
		_canScrollNext=true,
		_checkScrollStatus=function(){
			var div=_container[0],sL=div.scrollLeft!=0,sR=div.scrollWidth>div.clientWidth+div.scrollLeft,
		...
		_refreshScroll=function(redirect){
			var box=positionInContainer(_currentSelectedItem,_container),dis=_scrollDistance/2,compareWidth=config.scrollOnEdge?_unitWidth:0;
			if(box.right<compareWidth){
				if(redirect)_redirect(dis+Math.abs(box.right));
				else{
					scrollNext(dis+Math.abs(box.right));	
				}
				
			}
			if(box.left<compareWidth){
				if(redirect)_redirect(0-dis-Math.abs(box.left));
				else{
					scrollPrev(dis+Math.abs(box.left));
				}
			}
		},
		...
		_getScrollParams=function(sl){
			var div=_container[0],sl=(sl===undefined)?div.scrollLeft:Math.max(0,Math.min(div.scrollWidth,sl)),remain=div.scrollWidth-div.clientWidth-sl
			return {
				canPrev:sl!=0,
				canNext:remain>0,
		...
		_getVisibleItems=function(sl){//计算显示的元素，忽略padding
			var div=_container[0],l=(sl===undefined)?div.scrollLeft:sl,w=div.clientWidth,sw=div.scrollWidth,len=_getItemsLength(),
		...
		_redirect=function(dis){
			var div=_container[0],ori=div.scrollLeft;
			if(_scrollMutiple){
				dis=Math.round(dis/_unitWidth)*_unitWidth;
			}
			div.scrollLeft+=dis;
			if(div.scrollLeft!=ori){
				_scrollDoneSendEvent(dis>0?"next":"prev");
			}
			_checkScrollStatus();
		},
		scrollNext=function(dis){
			var div=_container[0],start=div.scrollLeft,dis=dis||_scrollDistance;
		...
		scrollPrev=function(dis){
			...
			_cacheIndex();
			//_refreshScroll(true);
			_container[0].scrollLeft+=_unitWidth*len;
			_selectedIndex+=len;
			_checkScrollStatus();
			return 0;
...
function doScroll(div,start,aim,callback){
	var aim=Math.min(div.scrollWidth-div.clientWidth,Math.max(aim,0)), dis=aim-start, span=Math.abs(dis), speed=span>50?(dis/5):(dis>0?5:-5);//加快速度，距离越远速度越大
	if(span>Math.abs(speed)){
		start+=speed;
		div.scrollLeft=start;
		setTimeout(function(){
